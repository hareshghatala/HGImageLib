...
open class SessionManager {
    ...
    /// A default instance of `SessionManager`, used by top-level HGNetworkLib request methods, and suitable for use
    ...
    public static let defaultHTTPHeaders: HTTPHeaders = {
        ...
        let userAgent: String = {
            if let info = Bundle.main.infoDictionary {
                ...
                let hgnetworklibVersion: String = {
                    guard
                        let afInfo = Bundle(for: SessionManager.self).infoDictionary,
                        let build = afInfo["CFBundleShortVersionString"]
                        else { return "Unknown" }
                    
                    return "HGNetworkLib/\(build)"
                }()
                
                return "\(executable)/\(appVersion) (\(bundle); build:\(appBuild); \(osNameVersion)) \(hgnetworklibVersion)"
            }
    ...
    let queue = DispatchQueue(label: "org.hgnetworklib.session-manager." + UUID().uuidString)
        ...
        DispatchQueue.global(qos: .utility).async {
            ...
            do {
                ...
                if formData.contentLength < encodingMemoryThreshold && !isBackgroundSession {
                    ...
                    let directoryURL = tempDirectoryURL.appendingPathComponent("org.hgnetworklib.manager/multipart.form.data")
